...
'@
            ...
            if ($Result.Success -and -not [string]::IsNullOrWhiteSpace($Result.Output)) {
                try {
                    $InstalledApps = $Result.Output | ConvertFrom-Json
                    ...
                    Write-Log `
                        -Level Warning `
                        -Message "Could not parse installed software list from $ComputerName"
                ...
                Write-Log `
                    -Level Warning `
                    -Message "Failed to query installed software on $ComputerName"
